...
export function buildEnvVars(env: OpenClawEnv): Record<string, string> {
  ...
  if (env.CLOUDFLARE_AI_GATEWAY_API_KEY) envVars.CLOUDFLARE_AI_GATEWAY_API_KEY = env.CLOUDFLARE_AI_GATEWAY_API_KEY;
  ...
  // Gateway token (container expects OPENCLAW_GATEWAY_TOKEN)
  ...
  // Map CLOUDFLARE_ACCOUNT_ID -> CF_ACCOUNT_ID (name expected by start-openclaw.sh)
